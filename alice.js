...
const isSibot = botNumber.includes(m.sender)
...
// REPLY NEWSLETTER
const fsaluran = { key : { remoteJid: '0@s.whatsapp.net', participant : '0@s.whatsapp.net' }, message: { newsletterAdminInviteMessage: { newsletterJid: global.idsal, newsletterName: '', caption: body }}}
// REPLY API
...
alice.sendPresenceUpdate("composing", m.chat)
}
...
const getGroups = await alice.groupFetchAllParticipating()
const groups = Object.keys(getGroups)
let text = '*[ System Notice ]* Alice secara otomatis membuka grup ini karena pagi hari.'
if (groups.some(v => v === from) && (await alice.groupMetadata(from)).announce) {
alice.groupSettingUpdate(from, 'not_announcement');
alice.sendMessage(from, {text: text, mentions: Object.values(global.db.groups[from].member).map(v => v.id)}, {ephemeralExpiration: m.expiration});
alice.groupRequestParticipantsList(from).then(async (data) => {
...
replyURL(`${tag} sedang *Afk* ${reason ? 'karena ' + reason : 'tanpa alasan'} selama *${clockString(new Date - afkTime)}*
`.trim())
}
if (global.db.users[sender].afkTime > -1) {
let user = global.db.users[sender]
replyURL(`${tag} telah kembali dari *Afk* ${user.afkReason ? 'setelah ' + user.afkReason : ''}\nselama *${clockString(new Date - user.afkTime)}*`.trim())
user.afkTime = -1
user.afkReason = ''
}
...
if (!roof.pilih2) alice.sendMessage(roof.p2, {text: '_Lawan sudah memilih_\nSekarang giliran kamu'})
}
...
alice.sendMessage(from, {text: tyu}, {quoted:m})

if (!roof.pilih) alice.sendMessage(roof.p, {text: '_Lawan sudah memilih_\nSekarang giliran kamu'})
}
...
await m.reply(messagee)
        if (res) await alice.sendMessage(m.chat, {
        audio: res,
        mimetype: 'audio/mp4',
        ptt: true,
...
// FITUR DOWNLOAD
case 'ttslide': {
    let input = `Ups penggunaan salah!
	
Example : ${prefix + command} https://vt.tiktok.com/blablabla/`
...
try {
    ...
    let no = 1
    for (let i of result) {
        alice.sendMessage(m.sender, {image: {url: i}, caption: `Gambar ke - ${no++}`}, {quoted: floc})
}
} catch (error) {
 m.reply(error)
}
...
${maximus.title_audio}
`;
await alice.sendMessage(m.chat, { video: { url: maximus.video[0] }, caption: caption })
await alice.sendMessage(m.chat, { audio: { url: maximus.audio[0] }, mimetype: "audio/mpeg", ptt: true }, { quoted: floc })
      } catch (e) {
		console.log(e)
		reply(e)
	}
...
case "ytplay": case "play": {
  if (!text) return reply("*Enter Title*\nexample : play coldplay yellow");
...
async function handleCommandytplay(text) {
    ...
    return alice.sendMessage(m.chat, doc, { quoted: fpayment2 });
}
...
if (!text) return m.reply(`Example : ${prefix + command} dog`)
...
for (const x of tahu) {
alice.sendMessage(m.chat, {image: {url: x}, caption: `Done`}, {quoted: m})
}
}
break
case 'gpt4': case 'alice': {
  if (!text) return m.reply(`Hai, alice disini, apa yang bisa saya bantu?`)
...
let kanjuthann = await openai(text, "nama mu adalah Alice, kamu adalah asisten kecerdasan buatan yang sering membantu orang lain jika ada yang ditanyakan, dan kamu adalah ksatria wanita suci")
...
break
case "pushkontakfrom":{
if (!isCreator&!isSibot&!isPremium) return reply('khusus pengguna bot saja')
